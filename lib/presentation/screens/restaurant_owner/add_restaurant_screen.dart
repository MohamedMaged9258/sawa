...
class _AddRestaurantScreenState extends State<AddRestaurantScreen> {
  ...
  final List<String> _cuisineTypes = ['Italian', 'Mexican', 'Chinese', 'Indian', 'American', 'Mediterranean', 'Other'];
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Restaurant added!'), backgroundColor: Colors.green));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Restaurant'), backgroundColor: Colors.orange[700], foregroundColor: Colors.white),
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Location', border: const OutlineInputBorder(),
                  suffixIcon: IconButton(icon: const Icon(Icons.my_location), onPressed: _getCurrentLocation),
                ),
                validator: (v) => v!.isEmpty ? 'Required' : null,
              ),
              const SizedBox(height: 16),
              TextFormField(controller: _priceController, decoration: const InputDecoration(labelText: 'Avg Price', border: OutlineInputBorder()), keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? 'Required' : null),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isSubmitting ? const CircularProgressIndicator(color: Colors.white) : const Text('Add Restaurant', style: TextStyle(color: Colors.white)),
                ),
              ),
            ],
          ),
