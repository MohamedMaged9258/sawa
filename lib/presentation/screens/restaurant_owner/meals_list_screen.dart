...
class _MealsListScreenState extends State<MealsListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Meals'), backgroundColor: Colors.blue[800], foregroundColor: Colors.white, elevation: 0),
      floatingActionButton: FloatingActionButton(onPressed: _showAddMealDialog, backgroundColor: Colors.blue[800], child: const Icon(Icons.add, color: Colors.white)),
      body: _isLoading 
        ? Center(child: CircularProgressIndicator(color: Colors.blue[800]))
        : _meals.isEmpty 
           ? Center(child: Text("No meals added", style: TextStyle(color: Colors.grey[600])))
           : ListView.builder(
                ...
                child: ListTile(
                  ...
                  leading: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: meal.photo.isNotEmpty ? DecorationImage(image: NetworkImage(meal.photo), fit: BoxFit.cover) : null
                    ),
                    child: meal.photo.isEmpty ? Icon(Icons.fastfood, color: Colors.blue[300]) : null,
                  ),
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        icon: Icon(meal.isAvailable ? Icons.check_circle : Icons.do_not_disturb_on, color: meal.isAvailable ? Colors.green : Colors.grey[400]),
...
class _AddMealDialogState extends State<AddMealDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
            ...
            children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: _image == null ? Icon(Icons.add_a_photo, color: Colors.blue[300]) : null,
                ),
              ),
              const SizedBox(height: 16),
              _isLoadingRestaurants 
                ? const CircularProgressIndicator()
                : _buildDropdown(_selectedRestaurantId, 'Restaurant', _restaurants.map((r) => DropdownMenuItem(value: r.rid, child: Text(r.name))).toList(), (v) => setState(() => _selectedRestaurantId = v)),
              ...
              const SizedBox(height: 12),
              _buildDropdown(_category, 'Category', _categories.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), (v) => setState(() => _category = v)),
            ],
          ),
        ),
      ),
      actions: [
        ...
        ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.blue[800]), onPressed: _submit, child: const Text('Add', style: TextStyle(color: Colors.white))),
      ],
    );
  }
  ...
  Widget _buildDropdown(dynamic val, String label, List<DropdownMenuItem<dynamic>> items, Function(dynamic) onChanged) {
